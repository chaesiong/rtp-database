CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "SYSTEM"."PKG_SEARCH" is
  --
  /*
  --
  ...
  */
  ...
  c_object_version   constant varchar2(19) := '1.0.0100';
    ...
    then
      ...
      then
        ...
        then
          v_result := v_result || '
                          --
                          and (' || v_query_function || '(t0.SURNAME)) = ' || v_query_function || '(''' || p_ppqry_surname || '''))
                          --
                          or ' || v_query_function || '(t5.TFAMILYNM)) = ' || v_query_function || '(''' || p_ppqry_surname || '''))
                          --
                          )';
        end if;
        --
        ...
        then
          v_result := v_result || '
                          --
                          and (' || v_query_function || '(t5.TMIDDLENM)) = ' || v_query_function || '(''' || p_ppqry_middle_name || '''))
                          --
                          )';
        end if;
        --
        ...
        then
          v_result := v_result || '
                          --
                          and (' || v_query_function || '(t0.GIVEN_NAME)) = ' || v_query_function || '(''' || p_ppqry_given_name || '''))
                          --
                          or ' || v_query_function || '(t5.TFIRSTNM)) = ' || v_query_function || '(''' || p_ppqry_given_name || '''))
                          --
                          )';
        end if;
        --
        ...
        then
          ...
          then
            v_result := v_result || '
                          --
                          and (t0.SEX = ''' || p_ppqry_sex || '''';
            --
            if (p_use_index_hints)
            then
              case p_query_type4where
                when 'LOWER_TRIM' then
                  v_result := v_result || '
                          --
                          or exists (select /*+ index(e0 ' || g_lt_idx4human_sexes || ') */ null';
                  --
                when 'SOUNDEX_TRIM' then
                  v_result := v_result || '
                          --
                          or exists (select /*+ index(e0 ' || g_st_idx4human_sexes || ') */ null';
                  --
                else
                  raise_application_error(-20000
                                         ,'Unknown p_Query_Type4Where[''' || p_query_type4where || ''']!');
            ...
            v_result := v_result || '
                                               ...
                                               where t0.SEX = e0.ICAO
                                                 and ' || v_query_function || '(e0.DISPLAY_VALUE)) = ' || v_query_function || '(''' || p_ppqry_sex || '''))
                                              --
                                              )
                          --
                          )';
          ...
          then
            ...
            then
              case p_query_type4where
                when 'LOWER_TRIM' then
                  v_result := v_result || '
                          --
                          or exists (select /*+ index(e1 ' || g_lt_idx4icao_doc_codes || ') */ null';
                  --
                when 'SOUNDEX_TRIM' then
                  v_result := v_result || '
                          --
                          or exists (select /*+ index(e1 ' || g_st_idx4icao_doc_codes || ') */ null';
                  --
                else
                  raise_application_error(-20000
                                         ,'Unknown p_Query_Type4Where[''' || p_query_type4where || ''']!');
            ...
            v_result := v_result || '
                                               ...
                                               where t0.NATIONALITY = e1.KEY_VALUE
                                                 and ' || v_query_function || '(e1.DISPLAY_VALUE)) = ' || v_query_function || '(''' || p_ppqry_nationality || '''))
                                              --
                                              )
                          --
                          )';
        ...
        then
          ...
          then
            case p_query_type4where
              when 'LOWER_TRIM' then
                v_result := v_result || '
                          --
                          or exists (select /*+ index(e2 ' || g_lt_idx4icao_doc_codes || ') */ null';
                --
              when 'SOUNDEX_TRIM' then
                v_result := v_result || '
                          --
                          or exists (select /*+ index(e2 ' || g_st_idx4icao_doc_codes || ') */ null';
                --
              else
                raise_application_error(-20000
                                       ,'Unknown p_Query_Type4Where[''' || p_query_type4where || ''']!');
          ...
          v_result := v_result || '
                                      ...
                                      where t0.ISSUING_COUNTRY = e2.KEY_VALUE
                                        and ' || v_query_function || '(e2.DISPLAY_VALUE)) = ' || v_query_function || '(''' || p_ppqry_tdt_issuing_country || '''))
                                    --
                                    )
                          --
                          )';
      ...
      then
        ...
        then
          v_result := v_result || '
                         --
                         and (' || v_query_function || '(t0.MIDDLE_NAME)) = ' || v_query_function || '(''' || p_ppqry_middle_name || '''))
                         --
                         or ' || v_query_function || '(t5.TMIDDLENM)) = ' || v_query_function || '(''' || p_ppqry_middle_name || '''))
                         --
                         )';
        end if;
        --
            ...
            then
              case p_query_type4where
                when 'LOWER_TRIM' then
                  v_result := v_result || '
                         --
                         or exists (select /*+ index(e1 ' || g_lt_idx4countries || ') */ null';
                  --
                when 'SOUNDEX_TRIM' then
                  v_result := v_result || '
                         --
                         or exists (select /*+ index(e1 ' || g_st_idx4countries || ') */ null';
                  --
                else
                  raise_application_error(-20000
                                         ,'Unknown p_Query_Type4Where[''' || p_query_type4where || ''']!');
    ...
    then
      ...
      then
        ...
        then
          v_result := v_result || '
                          ...
                          --
                          or ' || v_query_function || '(t5.TFAMILYNM)) like ' || v_query_function || '(''' || p_ppqry_surname || ''')) || ''%''
                          --
                          )';
        end if;
        --
        ...
        then
          v_result := v_result || '
                          --
                          or (' || v_query_function || '(t5.TMIDDLENM)) like ' || v_query_function || '(''' || p_ppqry_middle_name || ''')) || ''%''
                          --
                          )';
        end if;
        --
        ...
        then
          v_result := v_result || '
                          --
                          or (' || v_query_function || '(t0.GIVEN_NAME)) like ' || v_query_function || '(''' || p_ppqry_given_name || ''')) || ''%''
                          --
                          or ' || v_query_function || '(t5.TFIRSTNM)) like ' || v_query_function || '(''' || p_ppqry_given_name || ''')) || ''%''
                          --
                          )';
        end if;
        --
        ...
        then
          ...
          then
            ...
            v_result := v_result || '
                                               from DL_COMMON.HUMAN_SEXES#I18N e0
                                              where t0.SEX = e0.ICAO
                                                and ' || v_query_function || '(e0.DISPLAY_VALUE)) like ' || v_query_function || '(''' || p_ppqry_sex || ''')) || ''%''
                                                            --
                                                            )
                          --
                          )';
          ...
          then
            ...
            v_result := v_result || '
                                               from DL_COMMON.ICAO_DOC_CODES#I18N e1
                                              where t0.NATIONALITY = e1.KEY_VALUE
                                                and ' || v_query_function || '(e1.DISPLAY_VALUE)) like ' || v_query_function || '(''' || p_ppqry_nationality || ''')) || ''%''
                                             --
                                             )
                          --
                          )';
        ...
        then
          ...
          v_result := v_result || '
                                              from DL_COMMON.ICAO_DOC_CODES#I18N e2
                                             where t0.ISSUING_COUNTRY = e2.KEY_VALUE
                                               and ' || v_query_function || '(e2.DISPLAY_VALUE)) like ' || v_query_function || '(''' || p_ppqry_tdt_issuing_country || ''')) || ''%''
                                            --
                                            )
                          --
                          )';
      ...
      then
        ...
        then
          v_result := v_result || '
                         --
                         or (' || v_query_function || '(t0.MIDDLE_NAME)) like ' || v_query_function || '(''' || p_ppqry_middle_name || ''')) || ''%''
                         --
                         or ' || v_query_function || '(t5.TMIDDLENM)) like ' || v_query_function || '(''' || p_ppqry_middle_name || ''')) || ''%''
                         --
                         )';
  ...
  from (
    ...
    then
      case v_query_type4where
        when 'LOWER_TRIM' then
          v_result := v_result || '
                 select /*+ leading(t0 t5 t4) index(t0 ' || g_lt_idx4travel_docs || ') index(t5 ' || g_lt_idx4thaipassport || ') */ t4.KEY_VALUE as ID';
        when 'SOUNDEX_TRIM' then
          v_result := v_result || '
                 select /*+ leading(t0 t5 t4) index(t0 ' || g_st_idx4travel_docs || ') index(t5 ' || g_st_idx4thaipassport || ') */ t4.KEY_VALUE as ID';
        else
          raise_application_error(-20000
                                 ,'Unknown v_Query_Type4Where[''' || v_query_type4where || ''']!');
      end case;
      ...
      v_result := v_result || '
                        ...
                        ,case
                           when (lower(trim(''' || v_qry_surname || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t5.TFAMILYNM))
                                                              ,lower(trim(''' || v_qry_surname || ''')))
      ...
      v_result := v_result || '
                        ...
                        ,case
                           when (lower(trim(''' || v_qry_middle_name || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t5.TMIDDLENM))
      ...
      v_result := v_result || '
                        ...
                        ,case
                           when (lower(trim(''' || v_qry_given_name || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t5.TFIRSTNM))
                                                              ,lower(trim(''' || v_qry_given_name || ''')))
    ...
    then
      v_result := v_result || '
                        ,case
                           when (lower(trim(''' || v_qry_sex || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t0.SEX))
                                                              ,lower(trim(''' || v_qry_sex || ''')))
                         end as SEX$MSCORE
                        ,nvl((';
      if (v_use_index_hints)
      then
        v_result := v_result || '
                             select /*+ index(s2 ' || case
                      when (v_query_type4where = 'LOWER_TRIM') then
                       g_lt_idx4human_sexes
                      else
                       g_st_idx4human_sexes
                    end || ') */ max(case';
      else
        v_result := v_result || '
                             select max(case';
      ...
      v_result := v_result || '
                                           when (lower(trim(''' || v_qry_sex || ''')) is null) then
                                            ...
                                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(s2.DISPLAY_VALUE))
                                                                              ,lower(trim(''' || v_qry_sex || ''')))
                                         end)
      ...
      case v_search_type
        when 'SIMPLE' then
          v_result := v_result || '
                                and ' || v_query_function || '(s2.DISPLAY_VALUE)) = ' || v_query_function || '(''' || v_qry_sex || '''))';
        when 'CANDIDATES' then
          v_result := v_result || '
                                and ' || v_query_function || '(s2.DISPLAY_VALUE)) like ' || v_query_function || '(''' || v_qry_sex || case
                        when (v_qry_sex is not null) then
                         '%'
                        else
                         ''
                      end || '''))';
    ...
    then
      v_result := v_result || '
                        ,case
                           when (lower(trim(''' || v_qry_nationality || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t0.NATIONALITY))
                                                              ,lower(trim(''' || v_qry_nationality || ''')))
                         end as NATIONALITY$MSCORE
                        ,nvl((';
      if (v_use_index_hints)
      then
        v_result := v_result || '
                             select /*+ index(s1 ' || case
                      when (v_query_type4where = 'LOWER_TRIM') then
                       g_lt_idx4icao_doc_codes
                      else
                       g_st_idx4icao_doc_codes
                    end || ') */ max(case';
      else
        v_result := v_result || '
                             select max(case';
      ...
      v_result := v_result || '
                                           when (lower(trim(''' || v_qry_nationality || ''')) is null) then
                                            ...
                                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(s1.DISPLAY_VALUE))
                                                                              ,lower(trim(''' || v_qry_nationality || ''')))
                                         end)
      ...
      case v_search_type
        when 'SIMPLE' then
          v_result := v_result || '
                                and ' || v_query_function || '(s1.DISPLAY_VALUE)) = ' || v_query_function || '(''' || v_qry_nationality || '''))';
        when 'CANDIDATES' then
          v_result := v_result || '
                                and ' || v_query_function || '(s1.DISPLAY_VALUE)) like ' || v_query_function || '(''' || v_qry_nationality || case
                        when (v_qry_nationality is not null) then
                         '%'
                        else
                         ''
                      end || '''))';
    ...
    then
      v_result := v_result || '
                        ,case
                           when (lower(trim(''' || v_qry_tdt_issuing_country || ''')) is null) then
                            ...
                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(t0.ISSUING_COUNTRY))
                                                              ,lower(trim(''' || v_qry_tdt_issuing_country || ''')))
                         end as TDT_ISSUING_COUNTRY$MSCORE
                        ,nvl((';
      if (v_use_index_hints)
      then
        v_result := v_result || '
                             select /*+ index(s0 ' || case
                      when (v_query_type4where = 'LOWER_TRIM') then
                       g_lt_idx4icao_doc_codes
                      else
                       g_st_idx4icao_doc_codes
      ...
      v_result := v_result || '
                                           when (lower(trim(''' || v_qry_tdt_issuing_country || ''')) is null) then
                                            ...
                                            UTL_MATCH.' || v_res_mscore_algorithm || '_SIMILARITY(lower(trim(s0.DISPLAY_VALUE))
                                                                              ,lower(trim(''' || v_qry_tdt_issuing_country || ''')))
                                         end)
      ...
      case v_search_type
        when 'SIMPLE' then
          v_result := v_result || '
                                and ' || v_query_function || '(s0.DISPLAY_VALUE)) = ' || v_query_function || '(''' || v_qry_tdt_issuing_country || '''))';
        when 'CANDIDATES' then
          v_result := v_result || '
                                and ' || v_query_function || '(s0.DISPLAY_VALUE)) like ' || v_query_function || '(''' || v_qry_tdt_issuing_country || case
                        when (v_qry_tdt_issuing_country is not null) then
                         '%'
                        else
                         ''
                      end || '''))';
    ...
    */
    ...
    then
      case v_query_type4where
        when 'LOWER_TRIM' then
          v_result := v_result || '
                 select /*+ leading(t0 t5) index(t0 ' || g_lt_idx4identities || ') index(t5 ' || g_lt_idx4thaipassport || ') */ t0.KEY_VALUE as ID';
        when 'SOUNDEX_TRIM' then
          v_result := v_result || '
                 select /*+ leading(t0 t5) index(t0 ' || g_st_idx4identities || ') index(t5 ' || g_st_idx4thaipassport || ') */ t0.KEY_VALUE as ID';
        else
          raise_application_error(-20000
                                 ,'Unknown v_Query_Type4Where[''' || v_query_type4where || ''']!');
      end case;
