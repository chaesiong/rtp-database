...
'   background-color: #f5f9fd;',
'}',
'#FaceLive .FaceItemLayout .fc-item-header span:not(.fa)  {',
'   padding-left: 42px; ',
'}',
...
'    display:none;',
'}',
'.t-Form-inputContainer input.datepicker, .t-Form-inputContainer input.password, .t-Form-inputContainer input.popup_lov, .t-Form-inputContainer input.text_field, .t-Form-inputContainer input[type=text], .t-Form-inputContainer select.selectlist, .t-For'
||'m-inputContainer select.yes_no, .t-Form-inputContainer select[multiple], .t-Form-inputContainer span.display_only, .t-Form-inputContainer textarea.textarea, .u-TF-item--datepicker, .u-TF-item--select, .u-TF-item--text, .u-TF-item--textarea {',
'    font-size: 1.3rem;',
'}',
'.t-Form-label, .t-Region-body {',
'    font-size: 1.55rem;',
'}'))
...
,p_last_upd_yyyymmddhh24miss=>'20190718142431'
);
...
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(50303385284241104)
,p_name=>'P3_NIST_NATIONALITY'
,p_item_sequence=>60
,p_item_plug_id=>wwv_flow_api.id(92475359184792323)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'Y'
...
'    l_traveldoc_face_id   dl_blacklist.travel_docs.face_id%TYPE;',
'    l_traveldoc_new_image_id varchar2(2000);',
'    l_traveldoc_id_without_image varchar2(2000);',
'    v_SEQ_ID              INTEGER;',
'BEGIN ',
'    SELECT identity, face_id, key_value',
'    INTO l_identity, l_traveldoc_face_id, l_traveldoc_id_without_image',
...
'    ',
'    IF l_traveldoc_face_id IS NULL AND :P3_PASSPHOTO_ID IS NOT NULL THEN',
'        l_traveldoc_new_image_id := :P3_PASSPHOTO_ID;',
'    END IF;',
'    ',
...
'',
'            , p_c001            => l_identity',
'            , p_c002            => l_traveldoc_face_id',
'            , p_clob001         => :P3_CASE_JSON',
'            , p_blob001         => null',
'          );',
'    ',
'        l_url := APEX_UTIL.PREPARE_URL(',
'            p_url => ''f?p='' || l_app || '':70:''||l_session||''::YES:70:P70_IDENTITY_ID,P70_TRAVELDOC_IMAGE_ID,P70_TRAVELDOC_ID:'' || l_identity || '',''|| l_traveldoc_new_image_id || '',''||l_traveldoc_id_without_image ,',
...
' ',
'END;'))
,p_attribute_02=>'P3_PASSPORT_NUMBER,P3_PASSPORT_ISSUING_COUNTRY,P3_CASE_JSON,P3_PASSPHOTO_ID'
...
wwv_flow_api.create_page_da_event(
 p_id=>wwv_flow_api.id(50303143443241102)
...
wwv_flow_api.create_page_da_action(
 p_id=>wwv_flow_api.id(50303230889241103)
,p_event_id=>wwv_flow_api.id(50303143443241102)
,p_event_result=>'TRUE'
,p_action_sequence=>10
,p_execute_on_page_init=>'N'
,p_action=>'NATIVE_JAVASCRIPT_CODE'
...
'        apex.item(''P3_FIRSTNAME'').setValue(names[0])',
'        if(names[1]){',
'            apex.item(''P3_SURNAME'').setValue(names[1])',
'        }',
'    }',
'    else{',
'        var names = nameStr.split(''/'');',
'        var surname = names.shift();',
'        apex.item(''P3_FIRSTNAME'').setValue(names.join('' ''))',
'        apex.item(''P3_SURNAME'').setValue(surname)',
'    }',
'}',
'',
...
'            val = ''UNKNOWN''; break;',
'    }',
'    apex.item(''P3_SEX'').setValue(val);',
'}',
'if(descText[0][''047'']){',
'    apex.item(''P3_PASSPORT_NUMBER'').setValue(descText[0][''047''][0][0])',
'}',
'if(descText[0][''038'']){',
'    if(descText[0][''038''][0][0].split(''/'')[0] != ''undefined'' && descText[0][''038''][0][0].split(''/'')[0] != ''''){',
'       apex.item(''P3_NIST_NATIONALITY'').setValue(descText[0][''038''][0][0].split(''/'')[0]) ',
'    }',
'}',
'if(descText[0][''035'']){',
'    var dobStr = descText[0][''035''][0][0].replace(/-/g,'''');',
'    apex.item(''P3_DATE_OF_BIRTH'').setValue(dobStr.substr(6,2)+''/''+dobStr.substr(4,2)+''/''+dobStr.substr(0,4))',
'}',
'if(descText[0][''037'']){',
'    apex.item(''P3_PLACE_OF_BIRTH'').setValue(descText[0][''037''][0][0])',
'}',
'if(this.data.fileHeader[0].originatingAgencyIdentifier){',
'    apex.item(''P3_AUTHORITY'').setValue(this.data.fileHeader[0].originatingAgencyIdentifier)',
'}'))
,p_stop_execution_on_error=>'Y'
);
wwv_flow_api.create_page_da_event(
 p_id=>wwv_flow_api.id(50303487939241105)
,p_name=>'GetCountryForNistNationality'
,p_event_sequence=>220
,p_triggering_element_type=>'ITEM'
,p_triggering_element=>'P3_NIST_NATIONALITY'
,p_bind_type=>'bind'
,p_bind_event_type=>'change'
);
wwv_flow_api.create_page_da_action(
 p_id=>wwv_flow_api.id(50303563790241106)
,p_event_id=>wwv_flow_api.id(50303487939241105)
,p_event_result=>'TRUE'
,p_action_sequence=>10
,p_execute_on_page_init=>'N'
,p_action=>'NATIVE_EXECUTE_PLSQL_CODE'
...
'begin ',
'    select icao, icao ',
'    into :P3_NATIONALITY, :P3_PASSPORT_ISSUING_COUNTRY ',
'    from dl_common.icao_doc_codes$lc ',
'    where country like :P3_NIST_NATIONALITY||''%''  and rownum = 1;',
'',
'end;'))
,p_attribute_02=>'P3_NIST_NATIONALITY'
,p_attribute_03=>'P3_NATIONALITY,P3_PASSPORT_ISSUING_COUNTRY'
,p_attribute_04=>'N'
,p_stop_execution_on_error=>'Y'
,p_wait_for_result=>'Y'
);
