...
,p_last_upd_yyyymmddhh24miss=>'20190708150520'
);
...
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(50303664722241107)
,p_name=>'P80_DATE_OF_BIRTH'
,p_item_sequence=>30
,p_item_plug_id=>wwv_flow_api.id(3132031413378707)
...
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(50303713585241108)
,p_name=>'P80_NATIONALITY'
,p_item_sequence=>60
,p_item_plug_id=>wwv_flow_api.id(3132031413378707)
...
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(50303853504241109)
,p_name=>'P80_NATIONALITY_LOOKUP'
,p_item_sequence=>50
,p_item_plug_id=>wwv_flow_api.id(3132031413378707)
...
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(50304118013241112)
,p_name=>'P80_PASSPORT_NUMBER'
,p_item_sequence=>70
,p_item_plug_id=>wwv_flow_api.id(3132031413378707)
...
'        apex.item(''P80_FIRSTNAME'').setValue(names.join('' ''))',
'        apex.item(''P80_LASTNAME'').setValue(surname)',
'    }',
'}',
'',
...
'}',
'if(descText[0][''047'']){',
'    apex.item(''P80_PASSPORT_NUMBER'').setValue(descText[0][''047''][0][0])',
'}',
'if(descText[0][''038'']){',
'    var twoLetterCode = descText[0][''038''][0][0].split(''/'')[0];',
'    if(twoLetterCode != ''undefined'' && twoLetterCode != ''''){',
'      apex.item(''P80_NATIONALITY_LOOKUP'').setValue(twoLetterCode)  ',
'    }',
'}',
'if(descText[0][''035'']){',
'    var dobStr = descText[0][''035''][0][0].replace(/-/g,'''');',
'    apex.item(''P80_DATE_OF_BIRTH'').setValue(dobStr.substr(6,2)+''/''+dobStr.substr(4,2)+''/''+dobStr.substr(0,4))',
'}',
'if(this.data.fileHeader[0].originatingAgencyIdentifier){',
'    apex.item(''P80_AUTHORITY'').setValue(this.data.fileHeader[0].originatingAgencyIdentifier)',
'}'))
,p_stop_execution_on_error=>'Y'
);
wwv_flow_api.create_page_da_event(
...
'    TravelDoc.issuingCountry = Demographic.nationality;',
'    Demographic.traveldocs = [];',
'    if(Demographic.passportNumber && Demographic.passportNumber != ''''){',
...
wwv_flow_api.create_page_da_event(
 p_id=>wwv_flow_api.id(50303999528241110)
,p_name=>'GetCountryCode'
,p_event_sequence=>110
,p_triggering_element_type=>'ITEM'
,p_triggering_element=>'P80_NATIONALITY_LOOKUP'
,p_bind_type=>'bind'
,p_bind_event_type=>'change'
);
wwv_flow_api.create_page_da_action(
 p_id=>wwv_flow_api.id(50304061842241111)
,p_event_id=>wwv_flow_api.id(50303999528241110)
,p_event_result=>'TRUE'
,p_action_sequence=>10
,p_execute_on_page_init=>'N'
,p_action=>'NATIVE_EXECUTE_PLSQL_CODE'
...
'begin ',
'    select icao',
'    into :P80_NATIONALITY',
'    from dl_common.icao_doc_codes$lc ',
'    where country like :P80_NATIONALITY_LOOKUP||''%''  and rownum = 1;',
'',
'end;'))
,p_attribute_02=>'P80_NATIONALITY_LOOKUP'
,p_attribute_03=>'P80_NATIONALITY'
,p_attribute_04=>'N'
,p_stop_execution_on_error=>'Y'
,p_wait_for_result=>'Y'
);
