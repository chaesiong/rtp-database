CREATE OR REPLACE EDITIONABLE FUNCTION "PIBICSDM2"."GET_CAL_AGE_BOI" (
  DATE_PARAM IN VARCHAR2, SYSDTE IN DATE
) RETURN VARCHAR2 AS 

rs VARCHAR2(5) := '';
sys_year NUMBER(4) := to_char(SYSDTE, 'yyyy');
sys_month NUMBER(2) := to_char(SYSDTE, 'mm');
sys_date NUMBER(2) := to_char(SYSDTE, 'dd');
bir_date NUMBER(2);
bir_month NUMBER(2);
bir_year NUMBER(4);
...
END get_CAL_AGE_BOI;
/
  GRANT EXECUTE ON "PIBICSDM2"."GET_CAL_AGE_BOI" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."GET_CAL_AGE_BOI" TO "BIOSAADM";
