CREATE OR REPLACE EDITIONABLE FUNCTION "PIBICSDM2"."GET_DEPTABBNNMUNDERL2" (deptseqno_tmp Number, flag varchar2) return varchar2 is
...
begin
 ...
 if tmplevel = 1 then
    select null,null into enm1,tnm1 from department where dept_seqno=deptseqno_tmp ; 
    ...
    select deptlevel into level1 from department where dept_seqno = rel2;
     if level1 = 1 then
      select null,null into enm1,tnm1 from department where dept_seqno = rel2;
      end if;
 ...
 if level2 = 2 then
    ...
    if level2 = 1 then
      select null,null into enm1,tnm1 from department where dept_seqno = rel3;
      end if;
end if; 
 if flag = 'T' then
...
/
  GRANT EXECUTE ON "PIBICSDM2"."GET_DEPTABBNNMUNDERL2" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."GET_DEPTABBNNMUNDERL2" TO "BIOSAADM";
