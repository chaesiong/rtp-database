CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."TRANSFER_TM_MIDNIGHT_IMM61" IS
tmpVar NUMBER;
/******************************************************************************
...
file_ut UTL_FILE.FILE_TYPE;
   cursor c1 is
    --select * from tminout where to_number(to_char(credte, 'YYYYMMDD')) < to_number(to_char(sysdate, 'YYYYMMDD'));
     select * from tminout where credte between to_date('201611070000','YYYYMMDDHH24MI') and to_date('201611072000','YYYYMMDDHH24MI');
    rec_tm c1%rowtype;  
begin
 ...
 batchlogno_seq := batchlog_seq.nextval;
        ...
        values (batchlogno_seq, 'tm', '19', 'โอนข้อมูลเดินทางต่อวัน ทดสอบ '||to_char(sysdate-1, '(DD/MM/YYYY)'), null, 0, 0, 0, sysdate, null, '1', 'TM0001', sysdate, 'N', 0, 0, 0, sysdate);
        commit;
...
-- utl_file.put_line(file_ut,sysdateTxt||' Start Transfer Tminout into Tmmain .... !!!');utl_file.fflush(file_ut); 
 ...
 loop fetch c1 into rec_tm; exit when c1%notfound;
   ...
   insert into tmmain_imm61 (tmrunno, personid, tfirstnm, tmiddlenm, tfamilynm, efirstnm, emiddlenm, efamilynm, sex, nationcd, 
   ...
   values (TMLASTSEQNO.nextval, rec_tm.personid, rec_tm.tfirstnm, rec_tm.tmiddlenm, 
   ...
   insert into tmmain_imm61 (tmrunno, personid, tfirstnm, tmiddlenm, tfamilynm, efirstnm, emiddlenm, efamilynm, sex, nationcd, outseqno, outdte, 
   ...
   values (TMLASTSEQNO.nextval, rec_tm.personid, rec_tm.tfirstnm, rec_tm.tmiddlenm, rec_tm.tfamilynm, rec_tm.efirstnm, 
...
END TRANSFER_TM_MIDNIGHT_IMM61;
/
  GRANT EXECUTE ON "PIBICSDM2"."TRANSFER_TM_MIDNIGHT_IMM61" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."TRANSFER_TM_MIDNIGHT_IMM61" TO "BIOSAADM";
