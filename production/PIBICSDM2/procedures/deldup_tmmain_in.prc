CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."DELDUP_TMMAIN_IN" IS
tmpVar NUMBER;
BEGIN
declare
...
cursor c1 is       
          /*        
        select * from (
        select intdtno, intm6no, to_char(indte, 'yyyymmddhh24mi'), count('X') cnt, max(tmrunno) maxtm, min(tmrunno) mintm from tmmain where to_char(indte, 'yyyymmdd') = '20141014' 
        --and typedata <> 'PISCES' 
        group by intdtno, intm6no, to_char(indte, 'yyyymmddhh24mi')) where cnt = 2 ;
        */
        
        select * from tmmain where to_char(incredte, 'yyyymmdd') = '20150416' and inseqno in (
...
begin
     ...
     file_ut := UTL_FILE.FOPEN('LOGS_DIR','log_Deldup_tmmain_in'||sysdateFile||'.txt','W');
     utl_file.put_line(file_ut,sysdateTxt||' Start migrate duplicate data pibics@Tmmain in .... !!!');utl_file.fflush(file_ut);    
    open c1;
        loop
    ...
    commit;
     ...
     select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into sysdateTxt from dual;
     utl_file.put_line(file_ut,sysdateTxt||' End Process  migrate duplicate data pibics@Tmmain in .... !!!');utl_file.fflush(file_ut);
     utl_file.fclose(file_ut);
end;

END DELDUP_TMMAIN_IN;
/
  GRANT EXECUTE ON "PIBICSDM2"."DELDUP_TMMAIN_IN" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."DELDUP_TMMAIN_IN" TO "BIOSAADM";
