CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PROCESSVISARUN" IS 
tmpVar NUMBER;
BEGIN
declare
...
file_ut UTL_FILE.FILE_TYPE;
    cursor c1 is        
        --select to_char(visarundte, 'yyyymmdd') visarundte from tempdate where to_char(visarundte, 'yyyymmdd') between '20140830' and '20140831' order by visarundte asc;
...
begin
    ...
    v_pcsreasion := 'PROCESS VISARUN';
    file_ut := UTL_FILE.FOPEN('LOGS_DIR','log_PROCESSVISARUN'||sysdateFile||'.txt','W');
    utl_file.put_line(file_ut,sysdateTxt||' Start PROCESSVISARUN .... !!!');utl_file.fflush(file_ut); 
    open c1;
    ...
    values ( batchlogno_seq, 'PIBICS', '999', 'PROCESSVISARUN -'||+ rec_tm.visarundte, 0, toralrecords, 0, 0, sysdate, null, '1', 'ADMIN01', sysdate, 'N');     
    commit; 
            ...
            ***/     
            ...
            if foundvisarun = 'FOUND' or foundvisarunday = 'FOUND' then
             ...
             loop
                ...
                chkinoutdate := null;
                begin
                    /*if (itm.efamilynm is not null) and (itm.efirstnm is not null) and (itm.emiddlenm is not null) then
                        select vsrpersonseqno, inoutdte into checkmastervisarun, chkinoutdate from tmvisarunperson where 
                        efamilynm = itm.efamilynm and efirstnm = itm.efirstnm and emiddlenm =  itm.emiddlenm and 
                        nationcd = itm.nationcd and sex = itm.sex and birthdte = itm.birthdte and rownum = 1;
                    elsif (itm.efamilynm is not null) and (itm.efirstnm is not null) and (itm.emiddlenm is null) then
                        select vsrpersonseqno, inoutdte into checkmastervisarun, chkinoutdate from tmvisarunperson where 
                        efamilynm = itm.efamilynm and efirstnm = itm.efirstnm and emiddlenm is null and 
                        nationcd = itm.nationcd and sex = itm.sex and birthdte = itm.birthdte and rownum = 1;
                    elsif (itm.efamilynm is not null) and (itm.efirstnm is null) and (itm.emiddlenm is null) then
                        select vsrpersonseqno, inoutdte into checkmastervisarun, chkinoutdate from tmvisarunperson where 
                        efamilynm = itm.efamilynm and efirstnm is null and emiddlenm is null and 
                        nationcd = itm.nationcd and sex = itm.sex and birthdte = itm.birthdte and rownum = 1;
                    elsif (itm.efamilynm is null) and (itm.efirstnm is not null) and (itm.emiddlenm is null) then
                        select vsrpersonseqno, inoutdte into checkmastervisarun, chkinoutdate from tmvisarunperson where 
                        efamilynm is null and efirstnm is not null and emiddlenm is null and 
                        nationcd = itm.nationcd and sex = itm.sex and birthdte = itm.birthdte and rownum = 1;
                    end if;*/
                ...
                if checkmastervisarun is null then        
                    ...
                    itm.credte, itm.creterm, itm.remark, itm.tmrunno, itm.personid, null, visarun.invisatypecd, 
                    ...
                    visarundays = totaldiff, intype = itm.intype, typedata = itm.typedata, creusr = itm.creusr, credte = itm.credte, credterm = itm.creterm,
            ...
            if foundvisarun = 'NOTFOUND' and foundvisarunday = 'NOTFOUND' then
                ...
                begin
                        select vsrpersonseqno, inoutdte into checkmastervisarun, chkinoutdate 
                        from tmvisarunperson where passportno = visarun.tdtno and nationcd = visarun.nationcd and rownum = 1;
                exception when no_data_found then  
                    checkmastervisarun := null;
                    chkinoutdate := null;
                end;                
 ...
 select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into sysdateTxt from dual;
 utl_file.put_line(file_ut,sysdateTxt||' End PROCESSVISARUN .... !!!');utl_file.fflush(file_ut);
...
END PROCESSVISARUN;
/
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSVISARUN" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSVISARUN" TO "BIOSAADM";
