CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PROCESSTMPERSONTOWV30" IS
tmpVar NUMBER;
BEGIN
declare
...
file_ut UTL_FILE.FILE_TYPE;
    ...
    and invisatypecd = '0014' and nationcd in (select countcd from (
    select countcd from country where countcd in ('B11', 'K02', 'E08', 'B13', 'M10')
    union
    select countcd from visabynation where visatypecd = '0014' and DEFAULTVISA = 'Y')) and intdtno <> 'PASSPORT'
...
begin
    ...
    values ( batchlogno_seq, 'PIBICS', '999', 'PROCESSTMPERSONTOWV30', null, 0, 0, 0, sysdate, null, '1', 'ADMIN01', sysdate, 'N');     
    commit;  
 
    delete tmpersontowv30;
    commit;

    file_ut := UTL_FILE.FOPEN('LOGS_DIR','log_PROCESSTMPERSONTOWV30'||sysdateFile||'.txt','W');
    utl_file.put_line(file_ut,sysdateTxt||' Start PROCESSTMPERSONTOWV30 .... !!!');utl_file.fflush(file_ut); 
    open c1;
    ...
    <<itemtm_loop>> for itm in
     ...
     loop
        ...
        --addressfn := get_addrfn24(rec_tm.intdtno, rec_tm.nationcd);
         ...
         insert into tmpersontowv30 (tmseqno, efamilynm, efirstnm, emiddlenm, sex, birthdte, nationcd, passportno, amount, inoutdte, dept_seqno, tm6no, chkcapture, fn38, cardtype) 
        values 
 ...
 select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into sysdateTxt from dual;
 utl_file.put_line(file_ut,sysdateTxt||' End PROCESSTMPERSONTOWV30 .... !!!');utl_file.fflush(file_ut);
...
END PROCESSTMPERSONTOWV30;
/
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSTMPERSONTOWV30" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSTMPERSONTOWV30" TO "BIOSAADM";
