CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."IMMPORTPISDATA2TMMAINBATCH" IS
BEGIN
      DECLARE
      --DECLARE TYPE           
                        ...
                        IF SUBSTR(BUF, 0, 5) != 'START' AND SUBSTR(BUF, 0, 3) != 'END' THEN 
                            ...
                            ELSIF (PISCES.SEX = '2') THEN
                             ...
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER, 'END SUBSTRING ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --CHECK LENGTH
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER, 'LENGTH ' || LENGTH(trim(BUF)));
                              IF (LENGTH(trim(BUF))!=120) THEN
...
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER, '                    ---------------------------------------------');
                                    ...
                                    END;
                              END IF;
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER, 'COMMIT RECORD WHEN INSERT OR UPDATE ');            
            ...
            BEGIN
                SELECT BATCHLOGNO INTO BATCHLOGNOSEQ FROM PISCESBATCHTMP;
            EXCEPTION WHEN NO_DATA_FOUND THEN
                BATCHLOGNOSEQ := 0;
            END;
...
END IMMPORTPISDATA2TMMAINBATCH; 
/
  GRANT EXECUTE ON "PIBICSDM2"."IMMPORTPISDATA2TMMAINBATCH" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."IMMPORTPISDATA2TMMAINBATCH" TO "BIOSAADM";
