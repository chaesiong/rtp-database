CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PROCESSTMPERSONEX3090" IS
tmpVar NUMBER;
BEGIN
declare
...
file_ut UTL_FILE.FILE_TYPE;
    ...
    /*select '1' cardtype, intdtno tdtno, nationcd, count('X') cnt from tmmain where to_char(indte, 'yyyymmdd') between  to_char(sysdate - 120, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd')
    and invisatypecd = '0017' and nationcd =  'V03' and intdtno <> 'PASSPORT' 
    --and intdtno = 'B5887138'
    group by intdtno, nationcd
    having count('X') > 3
    union all*/
    select '1' cardtype, intdtno tdtno, nationcd, count('X') cnt from tmmain where to_char(indte, 'yyyymmdd') between  to_char(sysdate - 360, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd')
    and invisatypecd = '0018' and nationcd =  'K02' and intdtno <> 'PASSPORT' 
...
begin
    ...
    values ( batchlogno_seq, 'PIBICS', '999', 'PROCESSTMPERSONEX3090', null, 0, 0, 0, sysdate, null, '1', 'ADMIN01', sysdate, 'N');     
    commit;  
    ...
    if rec_tm.nationcd = 'K02' then
        begin
            ...
            and to_char(inoutdate, 'yyyymmdd') between  to_char(sysdate - 360, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd')  and visatypecd = '0018'
            and cardtype = '1'
            ),  tmcardout as (
            select tm6no, tdtno, cardtype, efullname, inoutdate, dept_seqno, visatypecd from vw_tmmain where tdtno = rec_tm.tdtno  and nationcd = rec_tm.nationcd
            and to_char(inoutdate, 'yyyymmdd') between  to_char(sysdate - 360, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd') and visatypecd = '0018'
    ...
    if rec_tm.nationcd = 'V03' then
        begin
            select sum(case when chkdiffdate > 1 then 1 else chkdiffdate end) into total from (
            with tmcardin as (
            select tm6no, tdtno, cardtype, efullname, inoutdate, dept_seqno from vw_tmmain where tdtno = rec_tm.tdtno  and nationcd = rec_tm.nationcd
            and to_char(inoutdate, 'yyyymmdd') between  to_char(sysdate - 120, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd') and visatypecd = '0017'
            and cardtype = '1'
            ),  tmcardout as (
            select tm6no, tdtno, cardtype, efullname, inoutdate, dept_seqno from vw_tmmain where tdtno = rec_tm.tdtno  and nationcd = rec_tm.nationcd
            and to_char(inoutdate, 'yyyymmdd') between  to_char(sysdate - 120, 'yyyymmdd') and to_char(sysdate, 'yyyymmdd') and visatypecd = '0017'
...
END PROCESSTMPERSONEX3090;
/
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSTMPERSONEX3090" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSTMPERSONEX3090" TO "BIOSAADM";
