CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."MIGRATEIMM_TEST" IS
tmpVar NUMBER;
BEGIN
declare
    cursor c_passtmp is select * from TESTPASSTEMP WHERE STATUS IS NULL;
        ...
        loop fetch c_passtmp into rec_passtmp; exit when c_passtmp%notfound; 
                ...
                LOOP
                    ...
                    ELSIF rec_passtmp.FLAG IS NULL THEN --Case ที่ภาพไม่ตรง
                       FOR rec_tmonline IN (select * from (select * from vw_tmonline where personid = rec_imm.personid and length(tdtno) = 7 and tdtno <> rec_passtmp.TDTNO  order by inoutdate desc) where rownum = 1)
                       --vw_tmpibics2011
                        LOOP
                            foundData := 'Y';
                            ...
                            /*IF rec_tmonline.tdtno IS NOT NULL THEN*/
                                ...
                                rec_tmonline.EMIDDLENM,rec_tmonline.SEX,rec_tmonline.BIRTHDTE,rec_tmonline.NATIONCD,rec_tmonline.TDTNO,rec_tmonline.PASSPORTDTE,
...
END MIGRATEIMM_TEST;
/
  GRANT EXECUTE ON "PIBICSDM2"."MIGRATEIMM_TEST" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."MIGRATEIMM_TEST" TO "BIOSAADM";
