CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PS_BEGINVOAOVERSTAY" IS
tmpVar NUMBER;
BEGIN
    DECLARE
    ...
    CURSOR c1 IS
        ...
        and to_char(visaexpdte, 'yyyymmdd') < to_char(sysdate, 'yyyymmdd')
    ...
    BEGIN
        ...
        LOOP FETCH c1 INTO rec_voa; EXIT WHEN c1%NOTFOUND;
            ...
            --DBMS_OUTPUT.PUT_LINE('1 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
            -- ตรวจสอบข้อมูลการเดินทางออก --
            FOR rec_card2 IN (select * from (select personid from vw_tmonline where tdtno = rec_voa.tdtno 
                ...
                and cardtype = '2'
                and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI')) order by inoutdate desc) where rownum = '1')
            LOOP
                flagfound := 'FOUND CARD2 tdtno and nationcd '|| rec_card2.personid;
            END LOOP;            
            
            
            --DBMS_OUTPUT.PUT_LINE('2 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi')); 
            if flagfound = 'NOTFOUND' then
                if rec_voa.efamilynm is not null and rec_voa.efirstnm is not null and rec_voa.emiddlenm is not null then 
                    ...
                    FOR rec_card2 IN (select * from (select personid from vw_tmonline where (tm6no = rec_voa.tm6no) and cardtype = '2' 
                    ...
                    and to_char(inoutdate, 'yyyymmdd') >= '20170101' and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI')) 
                    ...
                    FOR rec_card2 IN (select * from (select personid from vw_tmonline where (tm6no = rec_voa.tm6no) 
            ...
            if flagfound = 'NOTFOUND' then
                FOR rec_card2 IN (select * from (select personid from vw_tmonline where tdtno = rec_voa.tdtno and tm6no = rec_voa.tm6no and cardtype = '2'
                and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI')) order by inoutdate desc) where rownum = '1')
                LOOP
            ...
            if flagfound = 'NOTFOUND' then
                if rec_voa.efamilynm is not null and rec_voa.efirstnm is not null and rec_voa.emiddlenm is not null then 
                    --DBMS_OUTPUT.PUT_LINE('TEST LFM personid '|| rec_voa.personid);
                    FOR rec_card2 IN (select * from (select personid from vw_tmonline where (tdtno = rec_voa.tdtno) and cardtype = '2' 
                    and efamilynm = rec_voa.efamilynm and efirstnm = rec_voa.efirstnm and emiddlenm = rec_voa.emiddlenm and birthdte = rec_voa.birthdte
                    and to_char(inoutdate, 'yyyymmdd') >= '20170101' and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI')) 
                    order by inoutdate desc) where rownum = '1')
                    LOOP
            ...
            -- ตรวจสอบข้อมูลการเดินทางออก --
            --DBMS_OUTPUT.PUT_LINE('3 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));           
           ...
           if flagfound = 'NOTFOUND' then
            ...
            if flagfound = 'NOTFOUND' then
                FOR rec_tmfd IN (select * from tminformdead where tdtno = rec_voa.tdtno
                and (case when rec_voa.nationcd in ('C10', 'M24', 'H04') and nationcd in ('C10', 'M24', 'H04') then 1 
                when rec_voa.nationcd not in ('C10', 'M24', 'H04') and nationcd = rec_voa.nationcd then 1 else 0 end = 1))
                LOOP
           ...
           --DBMS_OUTPUT.PUT_LINE('4 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
            if flagfound = 'NOTFOUND' then
                FOR rec_card1 IN (select * from (select personid from vw_tmonline where tdtno = rec_voa.tdtno  
                and (case when rec_voa.nationcd in ('C10', 'M24', 'H04') and nationcd in ('C10', 'M24', 'H04') then 1 
                when rec_voa.nationcd not in ('C10', 'M24', 'H04') and nationcd = rec_voa.nationcd then 1 else 0 end = 1)                 
                and cardtype = '1'
                and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI')) order by inoutdate desc) where rownum = '1')
            ...
            --DBMS_OUTPUT.PUT_LINE('5 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));

            if flagfound = 'NOTFOUND' then
                if rec_voa.efamilynm is not null and rec_voa.efirstnm is not null and rec_voa.emiddlenm is not null then -- L F M
                    --DBMS_OUTPUT.PUT_LINE('TEST LFM personid '|| rec_voa.personid);
                    FOR rec_card1 IN (select * from (select personid from vw_tmonline where (tm6no = rec_voa.tm6no ) and cardtype = '1' 
                    and efamilynm = rec_voa.efamilynm and efirstnm = rec_voa.efirstnm and emiddlenm = rec_voa.emiddlenm and birthdte = rec_voa.birthdte
                    and to_char(inoutdate, 'yyyymmdd') >= '20170101' and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI'))
                    order by inoutdate desc) where rownum = '1')
                    LOOP
                    ...
                    --DBMS_OUTPUT.PUT_LINE('TEST LM personid '|| rec_voa.personid);
                    FOR rec_card1 IN (select * from (select personid from vw_tmonline where (tm6no = rec_voa.tm6no ) 
            ...
            if flagfound = 'NOTFOUND' then
                if rec_voa.efamilynm is not null and rec_voa.efirstnm is not null and rec_voa.emiddlenm is not null then -- L F M
                    --DBMS_OUTPUT.PUT_LINE('TEST LFM personid '|| rec_voa.personid);
                    FOR rec_card1 IN (select * from (select personid from vw_tmonline where (tdtno = rec_voa.tdtno) and cardtype = '1' 
                    and efamilynm = rec_voa.efamilynm and efirstnm = rec_voa.efirstnm and emiddlenm = rec_voa.emiddlenm and birthdte = rec_voa.birthdte
                    and to_char(inoutdate, 'yyyymmdd') >= '20170101' and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI'))
                    order by inoutdate desc) where rownum = '1')
                    LOOP
            ...
            --DBMS_OUTPUT.PUT_LINE('6 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
            if flagfound = 'NOTFOUND' then
                FOR rec_card1 IN (select * from (select personid from vw_tmonline where tdtno = rec_voa.tdtno and tm6no = rec_voa.tm6no and cardtype = '1'
                and to_number(to_char(inoutdate, 'YYYYMMDDHH24MI')) > to_number(to_char(rec_voa.inoutdte, 'YYYYMMDDHH24MI'))
                order by inoutdate desc) where rownum = '1')
                LOOP
            ...
            --DBMS_OUTPUT.PUT_LINE('61 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
            ...
            --DBMS_OUTPUT.PUT_LINE('7 voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
            if flagfound = 'NOTFOUND' then
                ...
                values (voaoverstay_seqno.nextval, rec_voa.personid, rec_voa.tm6no, rec_voa.inoutdte, rec_voa.cardtype, rec_voa.tfirstnm, rec_voa.tmiddlenm, rec_voa.tfamilynm,
                 ...
                 rec_voa.permit_seqno, rec_voa.flightprefix, rec_voa.flightnumber, rec_voa.deptcd, extPersonSeq, extPermitdte, extFlag, null, null, null, null, null, null, null, null, sysdate, null, 
                ...
                values (delvoaovs_seqno.nextval, rec_voa.personid, rec_voa.tm6no, rec_voa.inoutdte, rec_voa.cardtype, rec_voa.tfirstnm, rec_voa.tmiddlenm, rec_voa.tfamilynm, rec_voa.efirstnm, rec_voa.emiddlenm,
                 ...
                 rec_voa.flightprefix, rec_voa.flightnumber, rec_voa.deptcd, extPersonSeq, extPermitdte, extFlag, null, null, null, null, null, null, null, null, sysdate, sysdate, flagfound, sysdate);   
        ...
        EXCEPTION
       WHEN others THEN
            --DBMS_OUTPUT.PUT_LINE('ERROR voaseqno = ' || rec_voa.vosseqno || to_char(sysdate, 'dd/MM/yyyy hh24mi'));
           DBMS_OUTPUT.PUT_LINE(sqlerrm||'[Re-Gen Error]');
     null;--prompt(sqlerrm||'[Re-Gen Error]');
     
    END;
END PS_BEGINVOAOVERSTAY;
/
  GRANT EXECUTE ON "PIBICSDM2"."PS_BEGINVOAOVERSTAY" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PS_BEGINVOAOVERSTAY" TO "BIOSAADM";
