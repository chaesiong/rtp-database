CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."GEN_BKLEDIT" 
IS
  tmpVar NUMBER;
  /******************************************************************************
...
BEGIN
  DECLARE
    ...
    CURSOR c1
    IS
    /*select * from watchlist where cases_no in (
    select crime_no||'-'||crime_year||'-'||org_code||'-'||offender_no cases_no from vwwanted_polis_release_y) 
    select * from watchlist where cases_no in (
    select crime_no||'-'||crime_year||'-'||org_code||'-'||offender_no cases_no from vwwanted_polisdel);*/
    
    select * from watchlist where wlcd in (select wlcd from wllogrerun); 
...
END;
/
  GRANT EXECUTE ON "PIBICSDM2"."GEN_BKLEDIT" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."GEN_BKLEDIT" TO "BIOSAADM";
