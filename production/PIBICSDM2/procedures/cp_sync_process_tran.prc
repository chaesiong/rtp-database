CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."CP_SYNC_PROCESS_TRAN" (
...
begin
  ...
  pTB_DESTINATION  := 'TMP_SYNC_TRAN_' || pTABLE_NAME;  
  ...
  END IF;
   ...
   -- 3. Insert Data from Source to Destination (Pibics to Bio Temp) ---> With Condition Date for Scoppe Data!!!
  IF pCP_STATUS = 'Y'    
    THEN
    
      pCP_STATUS      := 'N';
      pLOG_METHOD     := 'InsertDataToTempTable';    
      CP_SYNC_INSERT_TABLE_REMOTE(pLOG_CLASS, pLOG_METHOD, pTABLE_NAME, pTB_SOURCE, pTB_DESTINATION, pDB_OWNER, 
                                  pCONDITION_NAME, pCONDITION_TYPE, pCONDITION_FORMAT, 
                                  pCONDITION_START_TIMESTAMP,pCONDITION_END_TIMESTAMP, pPROCESS_BY, pCP_STATUS); 
  ...
  IF pCP_STATUS = 'Y'    
    THEN
      ...
      CP_SYNC_UPDATE_SYNC_DATE(pLOG_CLASS, pLOG_METHOD,'2',pLAST_STATUS, pTABLE_NAME, pCONDITION_START_TIMESTAMP,pCONDITION_END_TIMESTAMP, pPROCESS_BY);
       
  END IF;
  
  
  -- 7. Close Database Link after use
  pLOG_METHOD  := 'CloseDatabaseLink'; 
  CP_SYNC_CLOSE_DATABASE_LINK(pLOG_CLASS, pLOG_METHOD, pTABLE_NAME, pDBLINK, pPROCESS_BY);
...
end CP_SYNC_PROCESS_TRAN;
/
  GRANT EXECUTE ON "PIBICSDM2"."CP_SYNC_PROCESS_TRAN" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."CP_SYNC_PROCESS_TRAN" TO "BIOSAADM";
