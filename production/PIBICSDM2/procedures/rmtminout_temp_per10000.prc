CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."RMTMINOUT_TEMP_PER10000" (start_yyyymm IN VARCHAR2, end_yyyymm IN VARCHAR2) IS
tmpVar NUMBER;
BEGIN
declare
    ...
    BEGIN
        ...
        start_date := start_yyyymm;
        end_date :=  end_yyyymm;
        
         select count('X') into v_totalRow from tminout_tmp where to_char(inoutdte, 'YYYYMM') between start_date and end_date;
        IF v_totalRow > 0 THEN
            ...
            values ( batchlogno_seq, 'HIST', '999', 'ลบ Tminout_tmp (PIBICS)  '|| start_date || ' - ' || end_date , null, 0, 0, 0, sysdate, null, '1', ' HIST01', sysdate, 'N');
            commit;
            -- End Insert batchlog
                
            FOR v_round IN 1..v_totalRound
            LOOP
                select count('X') into v_rowCommit from tminout_tmp where 
                (to_char(inoutdte, 'YYYYMM')  between start_date and end_date ) and rownum <= 10000;

                -- Start Delete data
                delete from tminout_tmp where 
                (to_char(inoutdte, 'YYYYMM')  between start_date and end_date ) and rownum <= 10000;
                commit;
        ...
        END IF;
            ...
            select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into sysdateTxt from dual;
            utl_file.put_line(file_ut,sysdatetxt||' Complete remove data Tminout_tmp PIBICS '|| start_date || ' - ' || end_date || ' History  ...!!!!'); 
   ...
   WHEN others THEN
        ...
        utl_file.fclose(file_ut);
   END;
END RMTMINOUT_TEMP_PER10000;
/
  GRANT EXECUTE ON "PIBICSDM2"."RMTMINOUT_TEMP_PER10000" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."RMTMINOUT_TEMP_PER10000" TO "BIOSAADM";
