CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."GEN_BLACKLISTMANUAL" IS
tmpVar NUMBER;
/******************************************************************************
   NAME:       GEN_BLACKLISTMANUAL
   PURPOSE:    

   REVISIONS:
   Ver        Date        Author           Description
   ---------  ----------  ---------------  ------------------------------------
   1.0        13/12/2012   Administrator       1. Created this procedure.

   NOTES:

   Automatically available Auto Replace Keywords:
      Object Name:     GEN_BLACKLISTMANUAL
      Sysdate:         13/12/2012
      Date and Time:   13/12/2012, 14:10:52, and 13/12/2012 14:10:52
...
BEGIN
  DECLARE
    ...
    CURSOR c1
    IS
    select * from watchlistlog where wlcd in ('235802') and logflag = 'D' order by logdte desc;
    rec_tm c1%rowtype;
  BEGIN
    ...
    WHEN c1%notfound;
      ...
      ( SELECT b.* FROM watchlistnmlog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
        INTO WATCHLISTNMLOG
      ...
      ( SELECT b.* FROM WLINDICATECARDLOG b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
      ...
      ( SELECT b.* FROM WLRELATIONlog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
        INTO WLRELATIONLOG
      ...
      ( SELECT b.* FROM WLCHARGElog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
        INTO WLCHARGELOG
      ...
      ( SELECT b.* FROM WATCHLISTPIClog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
      ...
      ( SELECT b.* FROM WATCHLISTDOClog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
      ...
      ( SELECT b.* FROM WLREMARKlog b WHERE WATCHLISTLOGID = rec_tm.WATCHLISTLOGID
      )
      LOOP
        INSERT
        INTO WLREMARKLOG
...
END GEN_BLACKLISTMANUAL;
/
  GRANT EXECUTE ON "PIBICSDM2"."GEN_BLACKLISTMANUAL" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."GEN_BLACKLISTMANUAL" TO "BIOSAADM";
