CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PS_BEGINCARDTYPEONE" IS
tmpVar NUMBER;
/******************************************************************************
...
BEGIN
  DECLARE
   ...
   select cardtype, tm6no, efamilynm, efirstnm, emiddlenm, tdtno, nationcd, birthdte, max(inoutdte) maxinoutdte 
   from immigration where to_char(inoutdte, 'YYYYMM')  between '201401' 
   and  to_char(sysdate, 'YYYYMM') and cardtype = '1' 
   ...
   group by  cardtype, tm6no, efamilynm, efirstnm, emiddlenm, tdtno, nationcd, birthdte;
    ...
    BEGIN
        ...
        values (batchlogno_seq, 'OVS', '18', 'BEGINCARDTYPEONE', null, 0, 0, 0, sysdate, null, '1', 'OVS001', sysdate, 'N', 0, 0, 0, sysdate);
        commit;
        
        file_ut := UTL_FILE.FOPEN('LOGS_DIR', 'log_PS_BEGINCARDTYPEONE' || sysdateFile || '.txt', 'W');
        utl_file.put_line(file_ut, sysdateTxt || ' Start PS_BEGINCARDTYPEONE .... !!!');
        ...
        LOOP FETCH c1 INTO rec_imm; EXIT WHEN c1%NOTFOUND;
           postfix := '';
            ...
            LOOP
                ...
                and to_char(inoutdte, 'YYYYMMDD')between '20140101' and '20150531'   and cardtype = '1' and nationcd <> 'T03' order by inoutdte desc) where rownum = 1)
                 LOOP
                    IF rec_imm_inner.inoutdte > rec_immover.inoutdte THEN
                        postfix := 'DELETE';
                        totaldelete := totaldelete + 1;
                        
                        insert into delimmoverstay (diovseqno, personid, tm6no, inoutdte, cardtype, tfirstnm, tmiddlenm, tfamilynm, efirstnm, emiddlenm, efamilynm, sex, birthdte, nationcd, tdtcd, 
                        ...
                        extpermitdte, extflag, chngperson_seqno, chngpermitdte, chngflag, rqsperson_seqno, rqspermitdte, rqsflag, fnperson_seqno, alien_seqno, processdte, processdeldte, flagdel,ovsdte) 
                        select diov_seqno.nextval, personid, tm6no, inoutdte, cardtype, tfirstnm, tmiddlenm, tfamilynm, efirstnm, emiddlenm, efamilynm, sex, birthdte, nationcd, tdtcd,  
                        tdtno, visatypecd, visaexpdte, passportdte, passportexpdte, updusr, upddte, updterm, remark, status, tm5_seqno, tm2inout_seqno, convcd, convregno, seqno, create_by, 
                        create_date, update_by, update_date, version, conv_seqno, visatype_seqno, dept_seqno, count_seqno, permit_seqno, flightprefix, flightnumber, deptcd, extperson_seqno, 
                        extpermitdte, extflag, chngperson_seqno, chngpermitdte, chngflag, rqsperson_seqno, rqspermitdte, rqsflag, fnperson_seqno, alien_seqno, processdte, sysdate, 'PS_BEGINCARDTYPEONE_1',ovsdte
                ...
                and to_char(inoutdte, 'YYYYMMDD')between '20140101' and '20150531'   and cardtype = '1' and nationcd <> 'T03' order by inoutdte desc) where rownum = 1)
                 LOOP
                    IF rec_imm_inner.inoutdte > rec_immover.inoutdte THEN
                        ...
                        tdtno, visatypecd, visaexpdte, passportdte, passportexpdte, updusr, upddte, updterm, remark, status, tm5_seqno, tm2inout_seqno, convcd, convregno, seqno, create_by, 
                        create_date, update_by, update_date, version, conv_seqno, visatype_seqno, dept_seqno, count_seqno, permit_seqno, flightprefix, flightnumber, deptcd, extperson_seqno, 
                        extpermitdte, extflag, chngperson_seqno, chngpermitdte, chngflag, rqsperson_seqno, rqspermitdte, rqsflag, fnperson_seqno, alien_seqno, processdte, sysdate, 'PS_BEGINCARDTYPEONE_2',ovsdte 
                        from immoverstay where personid = rec_immover.personid;
        ...
        utl_file.fflush(file_ut);
        utl_file.put_line(file_ut, sysdateTxt || ' End PS_BEGINCARDTYPEONE .... !!!');
        utl_file.fflush(file_ut);
...
--            --DBMS_OUTPUT.PUT_LINE(sqlerrm||'[Re-Gen Error]');
    ...
    END;
END PS_BEGINCARDTYPEONE;
/
  GRANT EXECUTE ON "PIBICSDM2"."PS_BEGINCARDTYPEONE" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PS_BEGINCARDTYPEONE" TO "BIOSAADM";
