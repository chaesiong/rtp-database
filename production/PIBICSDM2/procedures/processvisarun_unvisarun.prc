CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."PROCESSVISARUN_UNVISARUN" IS 
tmpVar NUMBER;
BEGIN
declare
...
file_ut UTL_FILE.FILE_TYPE;
    cursor c1 is        
        select to_char(visarundte, 'yyyymmdd') visarundte from tempdate where to_char(visarundte, 'yyyymmdd') between '20140401' and '20140415' order by visarundte desc;
...
begin
    ...
    file_ut := UTL_FILE.FOPEN('LOGS_DIR','log_PROCESSVISARUN_UNVISARUN'||sysdateFile||'.txt','W');
    utl_file.put_line(file_ut,sysdateTxt||' Start PROCESSVISARUN_UNVISARUN .... !!!');utl_file.fflush(file_ut); 
    open c1;
    ...
    values ( batchlogno_seq, 'PIBICS', '999', 'PROCESSVISARUN_UNVISARUN -'||+ rec_tm.visarundte, 0, toralrecords, 0, 0, sysdate, null, '1', 'ADMIN01', sysdate, 'N');     
    commit; 
    
        <<itemtm_loop>> for visarun in
            (
                select intdtno tdtno, nationcd, invisatypecd, to_number(to_char(indte, 'yyyymmdd')) indte from tmmain 
                where to_char(indte, 'yyyymmdd')  = rec_tm.visarundte
                and invisatypecd in ('0017', '0018', '0014') and intdtno <> 'PASSPORT' 
                --and invisatypecd = '0018' and intdtno <> 'PASSPORT'
                group by intdtno, nationcd, invisatypecd, to_number(to_char(indte, 'yyyymmdd'))
            )
        loop     
 ...
 select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into sysdateTxt from dual;
 utl_file.put_line(file_ut,sysdateTxt||' End PROCESSVISARUN_UNVISARUN .... !!!');utl_file.fflush(file_ut);
 utl_file.fclose(file_ut);
 --Exception
  --when others then
 -- null;--prompt(sqlerrm||'[Re-Gen Error]');
end;
END PROCESSVISARUN_UNVISARUN;
/
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSVISARUN_UNVISARUN" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."PROCESSVISARUN_UNVISARUN" TO "BIOSAADM";
