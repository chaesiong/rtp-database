CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."MOVEIMMIGRATIONNULL" IS
tmpVar NUMBER;
/******************************************************************************
...
file_ut UTL_FILE.FILE_TYPE;
    cursor c1 is
    --select * from immigration where rowid in (select corrupt_rowid from corrupt_lobs1);
    
    select * from tempimm where flag = 'IMGCORRUPT';
...
begin
 ...
 open c1;
    loop fetch c1 into rec_tm; exit when c1%notfound;    
        ...
        update immigration set imginout = rec_tm.imginout, imgpass = rec_tm.imgpass, updusr = 'ittipon', upddte = sysdate where personid = rec_tm.personid;
        ...
        insert into tempimm (
           ...
           pv_seqno, pd_seqno, zone_seqno, shift_seqno, flag, countrow) 
        values ( rec_tm.personid, rec_tm.tm6no, rec_tm.inoutdte, rec_tm.cardtype, rec_tm.tfirstnm, rec_tm.tmiddlenm, rec_tm.tfamilynm, rec_tm.efirstnm, rec_tm.emiddlenm, rec_tm.efamilynm,
         ...
         rec_tm.deptcd, rec_tm.visatypecdorg, rec_tm.rescerttype, rec_tm.respvcd, rec_tm.resnoyear, rec_tm.pv_seqno, rec_tm.pd_seqno, rec_tm.zone_seqno, rec_tm.shift_seqno, 'IMGCORRUPT', commitrow);        
         
         */

         --delete immigration where personid = rec_tm.personid;
...
END MOVEIMMIGRATIONNULL;
/
  GRANT EXECUTE ON "PIBICSDM2"."MOVEIMMIGRATIONNULL" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."MOVEIMMIGRATIONNULL" TO "BIOSAADM";
