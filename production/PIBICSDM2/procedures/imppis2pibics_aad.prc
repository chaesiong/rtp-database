CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."IMPPIS2PIBICS_AAD" IS
BEGIN
      DECLARE
      --DECLARE TYPE           
      ...
      BATCHLOGNO_AADSEQ NUMBER :=0;
      FILETYPE_AAD VARCHAR2(8);
      ...
      F_HANDLER_D UTL_FILE.FILE_TYPE;
      BUF VARCHAR2(1000);
      RECORD_INDEX NUMBER;
          
      --DECLARE FILE WRITE LOG
      FLOG_HANDLER_D UTL_FILE.FILE_TYPE;
...
BEGIN     
  ...
  BEGIN
    SELECT PISFILE, FILE_TYPE INTO FILENAMEPISCES, FILETYPE_AAD FROM PISCESBATCHTMP WHERE TYPE = 'D';
    
    BATCHLOGNO_AADSEQ := BATCHLOG_SEQ.NEXTVAL;
    ...
    VALUES
    (BATCHLOGNO_AADSEQ, 'PISCES', FILETYPE_AAD, FILENAMEPISCES, NULL, 
    0, 0, 0, SYSDATE, NULL, 
  ...
  END;
      ...
      --FOR REC IN (SELECT COLUMN_VALUE FILENAME FROM TABLE(SPLITSTRING('pis090101',',',1)) WHERE SUBSTR(COLUMN_VALUE,1,3) = 'pis') LOOP
            ...
            --CREATE LOG FILE
            FLOG_HANDLER_D := UTL_FILE.FOPEN(DIRECTORY_NAME, 'LOG_' || UPPER(REC.FILENAME) || '-' || TO_CHAR(SYSDATE, 'HH24MI') || '.LOG', 'W');
          
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START PROCESS AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            
      
          -- OPEN PISCES FILE
        --dbms_output.put_line(DIRECTORY_NAME||' = '|| REC.FILENAME);
        F_HANDLER_D := UTL_FILE.FOPEN_NCHAR(DIRECTORY_NAME, REC.FILENAME, 'R');
            
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    START FILENAME = ' || REC.FILENAME);
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    START FILENAME AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.FFLUSH(FLOG_HANDLER_D);
            ...
            --START READ FROM FILE
                  LOOP
                        --READ LINE
                        UTL_FILE.GET_LINE_NCHAR(F_HANDLER_D, BUF);

                        --CHECK START AND END FILES
                        IF SUBSTR(BUF, 0, 5) != 'START' AND SUBSTR(BUF, 0, 3) != 'END' THEN 
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START SUBSTRING ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --COUNT INDEX RECORD
                            ...
                            ELSIF (PISCES.SEX = '2') THEN
                              ...
                              END IF;
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'END SUBSTRING ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --CHECK LENGTH
                              IF (LENGTH(BUF)!=120) THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR LENGTH NOT EQUAL 120 = ' || BUF);
                              ...
                              IF (PISCES.CARDTYPE > 2) THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR CARDTYPE MORE THAN 2 = ' || BUF);
                              ...
                              IF (PISCES.TMNO IS NULL OR PISCES.TMNO = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR TMNO = ' || BUF);
                              ...
                              IF (PISCES.DATE_IMPORT IS NULL OR PISCES.DATE_IMPORT = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR DATE = ' || BUF);
                              ...
                              IF (PISCES.NATION IS NULL OR PISCES.NATION = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR NATION = ' || BUF);
                              ...
                              IF (PISCES.NAME IS NULL OR PISCES.NAME = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR NAME = ' || BUF);
                              ...
                              IF (PISCES.DOB IS NULL OR PISCES.DOB = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR DOB = ' || BUF);    
                              ...
                              IF (PISCES.PASSPORTNO IS NULL OR PISCES.PASSPORTNO = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR PASSPORTNO = ' || BUF);
                            ...
                            BEGIN
                              ...
                              EXCEPTION WHEN OTHERS THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR PORT = ' || BUF);
                    ...
                    --SPLIT NAME
                              ...
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHK DUPLICATE TMMAIN ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              PORT2PIBICS := NULL;
 ...
 BEGIN
                                  IF (PISCES.CARDTYPE = '1') THEN                                        
                                   ...
                                   IF (PISCES.CARDTYPE = '2') THEN
                                    ...
                                    IF (CHECKDUPTMRUNNO IS NOT NULL) THEN                                    
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR DUPLICATE RECORD = ' || BUF);                                        
                              ...
                              BEGIN
                                  IF (PISCES.CARDTYPE = '1') THEN
                                        IF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NOT NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 1 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME);
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 2 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME);
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 3 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME); 
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS  NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL)   THEN                                        
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 4 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3));
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                    ...
                                    ELSIF (PISCES.CARDTYPE = '2') THEN
                                        ...
                                        END IF; 
                                    END IF;
                                    --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '   CHECKDUPTMRUNNO = ' || CHECKDUPTMRUNNO);
                                    IF (CHECKDUPTMRUNNO IS NOT NULL) THEN                                    
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR DUPLICATE RECORD = ' || BUF);
                                          INSERT INTO PISLOGDETAILS VALUES(PLDSEQNO.NEXTVAL, REC.FILENAME, 'ERROR DUPLICATE RECORD', BUF, SYSDATE);
                                        CONTINUE;
                              ...
                              */
                              ...
                              BEGIN
                                    IF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NOT NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 1 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                    ...
                                    ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 2 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                        --||' SEX '||PISCES.SEX||' NATION '||PISCES.NATION||' DOB '||PISCES.DOB);
                                        SELECT PERSONID INTO PERSONIDFORDUP FROM IMMIGRATION WHERE 
                                        EFAMILYNM = NAMESPLIT(1) AND EFIRSTNM = NAMESPLIT(2) AND EMIDDLENM IS NULL AND 
                                        NATIONCD = PISCES.NATION AND SEX = PISCES.SEX AND BIRTHDTE = PISCES.DOB AND ROWNUM = 1;  
                                    ...
                                    ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'START CHECKDUP  CASE NAME 3 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                        --||' SEX '||PISCES.SEX||' NATION '||PISCES.NATION||' DOB '||PISCES.DOB);
                                        SELECT PERSONID INTO PERSONIDFORDUP FROM IMMIGRATION WHERE 
                                        EFAMILYNM = NAMESPLIT(1) AND EFIRSTNM IS NULL AND EMIDDLENM IS NULL AND 
                                        NATIONCD = PISCES.NATION AND SEX = PISCES.SEX AND BIRTHDTE = PISCES.DOB AND ROWNUM = 1;  
                              ...
                              END;
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'END CHK MASTER IMM ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));       
                              ----------------------------------------                                                                                   
                              -- BUILD SQL COMMAND
                              ...
                              BEGIN
                                  IF SQLCOMMAND_TMMAIN IS NOT NULL THEN    
                                    ...
                                    END IF;
                              --EXCEPTION WHEN OTHERS THEN
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ---------------------------------------------');
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR IN SQL = ' || SQLCOMMAND_TMMAIN);
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR IN TEXT = ' || BUF);
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ---------------------------------------------');
                                    UTL_FILE.FFLUSH(FLOG_HANDLER_D);
                              END;                                                                   
                              --------------------------------------------------------------------------------------------------------------
                               ...
                               --dbms_output.put_line('UPDATE START ');  
                                    ...
                                    ELSE
                                         ...
                                         --CONTINUE WHEN (TO_NUMBER(PISCES.DATE_IMPORT || PISCES.TIME) >=CHK_INOUTDTE);
                                          --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'CHK_INOUTDTE ' || CHK_INOUTDTE); 
                                          --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'CHK PISCES.DATE_IMPORT || PISCES.TIME ' || PISCES.DATE_IMPORT || PISCES.TIME);  
                                    ...
                                    --EXCEPTION WHEN OTHERS THEN
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ---------------------------------------------');
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR IN SQL = ' || SQLCOMMAND_IMMIGRATION);
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ERROR IN TEXT = ' || BUF);
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ---------------------------------------------');
                                          UTL_FILE.FFLUSH(FLOG_HANDLER_D);
                                    END;
                              END IF;
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'COMMIT RECORD WHEN INSERT OR UPDATE ');            
                  ...
                  END LOOP;
            EXCEPTION WHEN NO_DATA_FOUND THEN
                  UTL_FILE.FCLOSE(F_HANDLER_D);
                  COMMIT;
            END;
                          
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    INSERT ' || TABLE_TMMAIN_NAME || ' ' || COUNTER_INSERT_TMMAINTMP || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    INSERT ' || TABLE_IMMIGRATION_NAME || ' ' || COUNTER_INSERT_IMMIGRATION || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    UPDATE ' || TABLE_IMMIGRATION_NAME || ' ' || COUNTER_UPDATE_IMMIGRATION || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    COMMIT ALL ' || (COUNTER_INSERT_TMMAINTMP + COUNTER_INSERT_IMMIGRATION + COUNTER_UPDATE_IMMIGRATION) || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    END FILENAME = ' || REC.FILENAME);
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    FINNISH FILENAME AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, '                    ========================================================');
            UTL_FILE.FFLUSH(FLOG_HANDLER_D);
            ...
            --UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'CHECK_ISERROR: ' || COUNTER_ERROR);
            IF  COUNTER_ERROR > 0 THEN
                ...
                WHERE BATCHLOGNO = BATCHLOGNO_AADSEQ;   
            ...
            UTL_FILE.PUT_LINE(FLOG_HANDLER_D, 'FINNISH PROCESS AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.FFLUSH(FLOG_HANDLER_D);
            UTL_FILE.FCLOSE(FLOG_HANDLER_D);
...
END IMPPIS2PIBICS_AAD;
/
  GRANT EXECUTE ON "PIBICSDM2"."IMPPIS2PIBICS_AAD" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."IMPPIS2PIBICS_AAD" TO "BIOSAADM";
