CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."MIGRATEVOA2TMMAIN" IS
tmpVar NUMBER;
/******************************************************************************
...
begin
    ...
    dbms_output.put_line(timenow||' Start Process migrate voa 2 tmmain .... !!!');       
    open c1;
        loop
        fetch c1 into rec_tmmain; exit when c1%notfound;
        dbms_output.put_line('TMINOUT_SEQNO: '||rec_tmmain.tminout_seqno);
        update TMMAIN set nationcd = get_NationsBySeq(rec_tmmain.nation_seqno, 'S') where inseqno = to_char(rec_tmmain.tminout_seqno);
        commit;
        end loop;
    close c1;
    commit;
    select to_char(sysdate,'dd/mm/yyyy hh24:mi:ss') into timenow from dual;
    dbms_output.put_line(timenow||' Complete  Process migrate voa 2 tmmain .... !!!');
end;

END migrateVOA2TMMAIN;
/
  GRANT EXECUTE ON "PIBICSDM2"."MIGRATEVOA2TMMAIN" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."MIGRATEVOA2TMMAIN" TO "BIOSAADM";
