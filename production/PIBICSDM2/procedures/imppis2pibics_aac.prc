CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."IMPPIS2PIBICS_AAC" IS
BEGIN
      DECLARE
      --DECLARE TYPE           
      ...
      BATCHLOGNO_AACSEQ NUMBER :=0;
      FILETYPE_AAC VARCHAR2(8);
      ...
      F_HANDLER_C UTL_FILE.FILE_TYPE;
      BUF VARCHAR2(1000);
      RECORD_INDEX NUMBER;
          
      --DECLARE FILE WRITE LOG
      FLOG_HANDLER_C UTL_FILE.FILE_TYPE;
...
BEGIN     
  ...
  BEGIN
    SELECT PISFILE, FILE_TYPE INTO FILENAMEPISCES, FILETYPE_AAC FROM PISCESBATCHTMP WHERE TYPE = 'C';
    
    BATCHLOGNO_AACSEQ := BATCHLOG_SEQ.NEXTVAL;
    ...
    VALUES
    (BATCHLOGNO_AACSEQ, 'PISCES', FILETYPE_AAC, FILENAMEPISCES, NULL, 
    0, 0, 0, SYSDATE, NULL, 
  ...
  END;
      ...
      --FOR REC IN (SELECT COLUMN_VALUE FILENAME FROM TABLE(SPLITSTRING('pis090101',',',1)) WHERE SUBSTR(COLUMN_VALUE,1,3) = 'pis') LOOP
            ...
            --CREATE LOG FILE
            FLOG_HANDLER_C := UTL_FILE.FOPEN(DIRECTORY_NAME, 'LOG_' || UPPER(REC.FILENAME) || '-' || TO_CHAR(SYSDATE, 'HH24MI') || '.LOG', 'W');
          
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START PROCESS AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            
      
          -- OPEN PISCES FILE
        --dbms_output.put_line(DIRECTORY_NAME||' = '|| REC.FILENAME);
        F_HANDLER_C := UTL_FILE.FOPEN_NCHAR(DIRECTORY_NAME, REC.FILENAME, 'R');
            
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    START FILENAME = ' || REC.FILENAME);
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    START FILENAME AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.FFLUSH(FLOG_HANDLER_C);
            ...
            --START READ FROM FILE
                  LOOP
                        --READ LINE
                        UTL_FILE.GET_LINE_NCHAR(F_HANDLER_C, BUF);

                        --CHECK START AND END FILES
                        IF SUBSTR(BUF, 0, 5) != 'START' AND SUBSTR(BUF, 0, 3) != 'END' THEN 
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START SUBSTRING ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --COUNT INDEX RECORD
                            ...
                            ELSIF (PISCES.SEX = '2') THEN
                              ...
                              END IF;
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'END SUBSTRING ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --CHECK LENGTH
                              IF (LENGTH(BUF)!=120) THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR LENGTH NOT EQUAL 120 = ' || BUF);
                              ...
                              IF (PISCES.CARDTYPE > 2) THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR CARDTYPE MORE THAN 2 = ' || BUF);
                              ...
                              IF (PISCES.TMNO IS NULL OR PISCES.TMNO = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR TMNO = ' || BUF);
                              ...
                              IF (PISCES.DATE_IMPORT IS NULL OR PISCES.DATE_IMPORT = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR DATE = ' || BUF);
                              ...
                              IF (PISCES.NATION IS NULL OR PISCES.NATION = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR NATION = ' || BUF);
                              ...
                              IF (PISCES.NAME IS NULL OR PISCES.NAME = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR NAME = ' || BUF);
                              ...
                              IF (PISCES.DOB IS NULL OR PISCES.DOB = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR DOB = ' || BUF);    
                              ...
                              IF (PISCES.PASSPORTNO IS NULL OR PISCES.PASSPORTNO = '') THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR PASSPORTNO = ' || BUF);
                            ...
                            BEGIN
                              ...
                              EXCEPTION WHEN OTHERS THEN
                                  --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR PORT = ' || BUF);
                    ...
                    --SPLIT NAME
                              ...
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHK DUPLICATE TMMAIN ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              PORT2PIBICS := NULL;
                            ...
                            BEGIN
                                  IF (PISCES.CARDTYPE = '1') THEN                                        
                                   ...
                                   IF (PISCES.CARDTYPE = '2') THEN
                                    ...
                                    IF (CHECKDUPTMRUNNO IS NOT NULL) THEN                                    
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR DUPLICATE RECORD = ' || BUF);                                        
                              ...
                              BEGIN
                                  IF (PISCES.CARDTYPE = '1') THEN
                                        IF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NOT NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 1 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME);
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 2 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME);
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 3 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)||' '||PISCES.DATE_IMPORT || PISCES.TIME); 
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                        ...
                                        ELSIF (NAMESPLIT(1) IS  NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL)   THEN                                        
                                            --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 4 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3));
                                            SELECT TMRUNNO INTO CHECKDUPTMRUNNO FROM TMMAIN T1 WHERE 
                                    ...
                                    ELSIF (PISCES.CARDTYPE = '2') THEN
                                        ...
                                        END IF; 
                                    END IF;
                                    --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '   CHECKDUPTMRUNNO = ' || CHECKDUPTMRUNNO);
                                    IF (CHECKDUPTMRUNNO IS NOT NULL) THEN                                    
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR DUPLICATE RECORD = ' || BUF);
                                          INSERT INTO PISLOGDETAILS VALUES(PLDSEQNO.NEXTVAL, REC.FILENAME, 'ERROR DUPLICATE RECORD', BUF, SYSDATE);
                                        CONTINUE;
                              ...
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHK MASTER IMM ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
                              --CHECK IMMIGRATION DUPPLICATE
                              BEGIN
                                    IF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NOT NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 1 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                    ...
                                    ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NOT NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 2 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                        --||' SEX '||PISCES.SEX||' NATION '||PISCES.NATION||' DOB '||PISCES.DOB);
                                        SELECT PERSONID INTO PERSONIDFORDUP FROM IMMIGRATION WHERE 
                                        EFAMILYNM = NAMESPLIT(1) AND EFIRSTNM = NAMESPLIT(2) AND EMIDDLENM IS NULL AND 
                                        NATIONCD = PISCES.NATION AND SEX = PISCES.SEX AND BIRTHDTE = PISCES.DOB AND ROWNUM = 1;  
                                    ...
                                    ELSIF (NAMESPLIT(1) IS NOT NULL) AND (NAMESPLIT(2) IS NULL) AND (NAMESPLIT(3) IS NULL) THEN
                                        --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'START CHECKDUP  CASE NAME 3 ' || NAMESPLIT(1)|| ' ' ||NAMESPLIT(2) || ' ' ||NAMESPLIT(3)
                                        --||' SEX '||PISCES.SEX||' NATION '||PISCES.NATION||' DOB '||PISCES.DOB);
                                        SELECT PERSONID INTO PERSONIDFORDUP FROM IMMIGRATION WHERE 
                                        EFAMILYNM = NAMESPLIT(1) AND EFIRSTNM IS NULL AND EMIDDLENM IS NULL AND 
                                        NATIONCD = PISCES.NATION AND SEX = PISCES.SEX AND BIRTHDTE = PISCES.DOB AND ROWNUM = 1;  
                              ...
                              END;
                              --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'END CHK MASTER IMM ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));       
                              ----------------------------------------                                                                                   
                              -- BUILD SQL COMMAND
                              ...
                              BEGIN
                                  IF SQLCOMMAND_TMMAIN IS NOT NULL THEN    
                                    ...
                                    END IF;
                              --EXCEPTION WHEN OTHERS THEN
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ---------------------------------------------');
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR IN SQL = ' || SQLCOMMAND_TMMAIN);
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR IN TEXT = ' || BUF);
--                                    UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ---------------------------------------------');
                                    UTL_FILE.FFLUSH(FLOG_HANDLER_C);
                              END;                                                                   
                              --------------------------------------------------------------------------------------------------------------
                               ...
                               --dbms_output.put_line('UPDATE START ');  
                                    ...
                                    ELSE
                                         ...
                                         --CONTINUE WHEN (TO_NUMBER(PISCES.DATE_IMPORT || PISCES.TIME) >=CHK_INOUTDTE);
                                          --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'CHK_INOUTDTE ' || CHK_INOUTDTE); 
                                          --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'CHK PISCES.DATE_IMPORT || PISCES.TIME ' || PISCES.DATE_IMPORT || PISCES.TIME);  
                                    ...
                                    --EXCEPTION WHEN OTHERS THEN
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ---------------------------------------------');
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR IN SQL = ' || SQLCOMMAND_IMMIGRATION);
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ERROR IN TEXT = ' || BUF);
--                                          UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ---------------------------------------------');
                                          UTL_FILE.FFLUSH(FLOG_HANDLER_C);
                                    END;
                              END IF;
                             --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'COMMIT RECORD WHEN INSERT OR UPDATE ');            
                  ...
                  END LOOP;
            EXCEPTION WHEN NO_DATA_FOUND THEN
                  UTL_FILE.FCLOSE(F_HANDLER_C);
                  COMMIT;
            END;
                          
            --WRITE LOG
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    INSERT ' || TABLE_TMMAIN_NAME || ' ' || COUNTER_INSERT_TMMAINTMP || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    INSERT ' || TABLE_IMMIGRATION_NAME || ' ' || COUNTER_INSERT_IMMIGRATION || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    UPDATE ' || TABLE_IMMIGRATION_NAME || ' ' || COUNTER_UPDATE_IMMIGRATION || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    COMMIT ALL ' || (COUNTER_INSERT_TMMAINTMP + COUNTER_INSERT_IMMIGRATION + COUNTER_UPDATE_IMMIGRATION) || ' RECORD(S)');
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    END FILENAME = ' || REC.FILENAME);
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    FINNISH FILENAME AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, '                    ========================================================');
            UTL_FILE.FFLUSH(FLOG_HANDLER_C);
            ...
            --UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'CHECK_ISERROR: ' || COUNTER_ERROR);
            IF  COUNTER_ERROR > 0 THEN
                ...
                WHERE BATCHLOGNO = BATCHLOGNO_AACSEQ;   
            ...
            UTL_FILE.PUT_LINE(FLOG_HANDLER_C, 'FINNISH PROCESS AT ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS'));
            UTL_FILE.FFLUSH(FLOG_HANDLER_C);
            UTL_FILE.FCLOSE(FLOG_HANDLER_C);
...
END IMPPIS2PIBICS_AAC;
/
  GRANT EXECUTE ON "PIBICSDM2"."IMPPIS2PIBICS_AAC" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."IMPPIS2PIBICS_AAC" TO "BIOSAADM";
