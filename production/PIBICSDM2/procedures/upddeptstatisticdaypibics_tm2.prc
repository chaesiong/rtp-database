CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."UPDDEPTSTATISTICDAYPIBICS_TM2" (S_DATE IN VARCHAR2, E_DATE IN VARCHAR2,STATISTICFILED  IN VARCHAR2,P_CARDTYPE  IN VARCHAR2) IS
tmpVar NUMBER;
/******************************************************************************
...
file_ut UTL_FILE.FILE_TYPE;
   ...
   with departmentlevel as (
    ...
    (
    select GET_DEPTCD(iodeptth) deptcd, iodeptth dept_seqno, count(tm2_seqno) amount from tm2
    where to_char(iodte, 'YYYYMMDDHH24MISS') between S_DATE and E_DATE 
    and cardtype = decode(P_CARDTYPE,'A',cardtype,P_CARDTYPE)
    group by GET_DEPTCD(iodeptth), iodeptth
    ) b
...
END UPDDEPTSTATISTICDAYPIBICS_TM2;
/
  GRANT EXECUTE ON "PIBICSDM2"."UPDDEPTSTATISTICDAYPIBICS_TM2" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."UPDDEPTSTATISTICDAYPIBICS_TM2" TO "BIOSAADM";
