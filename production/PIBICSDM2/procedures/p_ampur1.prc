CREATE OR REPLACE EDITIONABLE PROCEDURE "PIBICSDM2"."P_AMPUR1" (tstart varchar,tend varchar)
IS
...
BEGIN
   ...
   MERGE INTO PIBICSDM2.AMPUR DM
        USING (SELECT * 
                    FROM PIBICS.AMPUR@PIBICS_PROD
                    WHERE CREATE_DATE >= TO_DATE(v_dstart,'yyyymmdd') 
                        AND CREATE_DATE < TO_DATE(v_dend,'yyyymmdd')
                        AND UPDATE_DATE IS NULL) P
        ON (DM.AMP_SEQNO = P.AMP_SEQNO)
    WHEN MATCHED 
    THEN 
        UPDATE SET DM.AMPCD = P.AMPCD,
                    DM.PV_SEQNO = P.PV_SEQNO,
                    DM.AMP_DESC = P.AMP_DESC,
   ...
   INSERT INTO IMPORT_LOG
        VALUES ('AMPUR_C', var_rows, SYSDATE);
   ...
   INSERT INTO IMPORT_LOG
        VALUES ('AMPUR_U', var_rows, SYSDATE);
...
END P_AMPUR1;
/
  GRANT EXECUTE ON "PIBICSDM2"."P_AMPUR1" TO "APPSUP";
  GRANT EXECUTE ON "PIBICSDM2"."P_AMPUR1" TO "BIOSAADM";
