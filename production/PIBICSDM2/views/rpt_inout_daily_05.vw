CREATE OR REPLACE FORCE EDITIONABLE VIEW "PIBICSDM2"."RPT_INOUT_DAILY_05" ("AIRPORT", "COUNTER", "DIRECTION", "SEX", "NATIONALITY", "VISATYPE", "PERSONTYPE", "COUNTTRAVLER", "DATE_OF_ENTRY") AS 
...
,to_char(DATE_OF_ENTRY,'MON') as DATE_OF_ENTRY
FROM (
...
and TRUNC(m.DATE_OF_ENTRY) >= DATE '2019-05-01'
and TRUNC(m.DATE_OF_ENTRY) < DATE '2019-08-22'
order BY m.DATE_OF_ENTRY
) X
GROUP BY X.NAME, X.DISPLAY_VALUE, X.TNAME, X.SEX, X.NAT, X.EXITFLG, X.PERSON_TYPE,to_char(DATE_OF_ENTRY,'MON');
