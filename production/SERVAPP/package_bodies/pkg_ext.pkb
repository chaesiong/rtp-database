...
SELECT 
	...
	, (SELECT COUNT_SEQNO FROM PIBICSDM2.COUNTRY WHERE ABBCOUNT = p.NATIONALITY and actflag ='Y') AS  NATION_SEQNO
...
begin
PASSPORT_PIC_VAL := l_obj.get_object('recordInfo').get('PASSPORT_PIC').to_blob;
C_PASSPORT_PIC_VAL := DL_COMMON.PKG_UTIL.BLOB2CLOB(PASSPORT_PIC_VAL);
PASSPORT_PIC_VAL:=FN_CLOB_TO_BLOB(C_PASSPORT_PIC_VAL);

EXCEPTION WHEN OTHERS THEN
 PASSPORT_PIC_VAL := null;
end;

begin
EXTIMG_VAL := l_obj.get_object('recordInfo').get('EXTIMG').to_blob;
C_EXTIMG_VAL := DL_COMMON.PKG_UTIL.BLOB2CLOB(EXTIMG_VAL);
...
--,d.EXT_SEQNO=s.EXT_SEQNO
--,d.EXTPERSON_SEQNO=s.EXTPERSON_SEQNO
d.DOC_NO=nvl(s.DOC_NO,d.DOC_NO)
,d.REASON_SEQNO=s.REASON_SEQNO
,d.EXT_DAY=s.EXT_DAY
...
SELECT  FEES_SEQNO_VAL, FEESNO_VAL, FEESLIPNO_VAL,FEESLIPBOOKNO_VAL,SAVE_DATE_VAL FEESLIPDATE, FEESLIPTOTAL_VAL
   , f.PRINTFLAG
   , DEPT_SEQNO_VAL
   , f.CPASSPORTNO, f.TM6NO, f.EFIRSTNM, f.EMIDDLENM, f.EFAMILYNM, f.SEX, f.BIRTH_DATE
   , f.NATION_SEQNO
...
SELECT  FEES_SEQNO_VAL as FEES_SEQNO
   ...
   , f.EFAMILYNM, f.SEX, f.BIRTH_DATE  as BIRTHDTE
   , f.NATION_SEQNO
   , f.CONV_SEQNO, f.CONVREGNO, f.IN_DATE AS INDTE
   , f.VISATYPE_SEQNO, 'ACTIVE' AS FEESSTATUS, NULL AS FEESREMARK, SYSTEM_CODE_VAL || ' ' || MODULE_VAL AS FLAG_SYSTEM 
...
END PKG_EXT;
/
  GRANT EXECUTE ON "SERVAPP"."PKG_EXT" TO "BIOSAADM";
