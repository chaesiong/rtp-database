CREATE OR REPLACE EDITIONABLE PROCEDURE "SERVAPP"."P_TMINOUT" (
    v_dstart VARCHAR2,
    v_dend VARCHAR2
)
AS 
BEGIN
 ...
 INSERT INTO IMPORT_LOG
  VALUES ('TMINOUT_C_start', var_rows, SYSDATE);

 MERGE INTO SERVAPP.MSCS_TMINOUT DM
  USING (SELECT * 
     FROM pibicsdm2.TMINOUT
     --WHERE TO_CHAR(CREDTE,'yyyymmddhh24mi') >= v_dstart
      --AND TO_CHAR(CREDTE,'yyyymmddhh24mi') < v_dend) P
 ...
 INSERT INTO IMPORT_LOG
  VALUES ('TMINOUT_C_end', var_rows, SYSDATE);

 COMMIT;


INSERT INTO IMPORT_LOG
  VALUES ('TMINOUT_U_start', var_rows, SYSDATE);

 MERGE INTO SERVAPP.MSCS_TMINOUT DM
  USING (SELECT * 
     FROM pibicsdm2.TMINOUT
     WHERE TO_CHAR(UPDDTE,'yyyymmddhh24mi') >= v_dstart
      AND TO_CHAR(UPDDTE,'yyyymmddhh24mi') < v_dend
      AND TYPEDATA <> 'BIO') P
  ON (DM.SEQNO = P.SEQNO)
 WHEN MATCHED 
 THEN 
...
VALUES (     P.SEQNO,
 ...
 INSERT INTO IMPORT_LOG
  VALUES ('TMINOUT_U_end', var_rows, SYSDATE);
...
/
  GRANT EXECUTE ON "SERVAPP"."P_TMINOUT" TO "BIOSAADM";
